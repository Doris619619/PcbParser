...
// —— 目录常量 ——
// 按你的机器路径来
static const std::string kWF = "/home/tkwu/EDATools/workflow";
static const std::string kRR = "/home/tkwu/EDATools/rerouting";
static const std::string kP5 = "/home/tkwu/EDATools/Part5";

// rerouting 只认这一个输出文件名（你点名的那一份）
static const std::string kRR_OUT_NAME =
  "output.bestSolutionWithMerging.s_10_i_5_b_0_lc_500_to_15000_vo_40_po_2000_tss_0_vss_0.complete_pcb_template.kicad_pcb";

// 小工具：目录/复制/取环境变量
static void CopyOverwrite(const std::string& src, const std::string& dst) {
  fs::create_directories(fs::path(dst).parent_path());
  fs::copy_file(src, dst, fs::copy_options::overwrite_existing);
}
static std::string getenvOr(const char* k, const std::string& dflt) {
  if (const char* v = std::getenv(k)) return std::string(v);
  return dflt;
}
// 方便：拼一次“本轮输出目录”，形如 /home/.../Part5/output/iter_01/
//static std::string buildIterOutDir() {
//  std::string iter = getenvOr("P5_ITER", "01");
//  std::string outDir = kP5 + "/output/iter_" + iter + "/";
...
//==========================================全局常量与小工具==========================================
// ===== 统一的输出根目录：从环境变量取（默认 Part5/output/iter_XX/）=====
static std::string GetOutputDir() {
    const char* p5_dir = std::getenv("P5_DIR");   // 例如 /home/tkwu/EDATools/Part5
    const char* iter = std::getenv("P5_ITER");  // 例如 02
    std::string base = p5_dir ? p5_dir : ".";
    std::string it = iter ? iter : "00";
    std::string out = base + "/output/iter_" + it + "/";
...
// === 等待 rerouting 输出为“新且稳定”的文件（C++17 版，无需 to_time_t） ===
// 说明：C++17 的 filesystem 时间戳不是 system_clock，直接 to_time_t 往往不兼容，所以
// 我们用 file_time_type 自己的时钟做“新旧比较”，避免 to_time_t 报错。
// === 等待 rerouting 输出成为“新或已变化并且大小稳定”的文件（考虑 1s mtime 分辨率） ===
static bool WaitForNewOrChangedStableFile(
    const std::string& path,
    bool pre_exists,
    fs::file_time_type pre_ft,
    uintmax_t pre_size,
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        try {
            if (fs::exists(path)) {
                ...
                bool is_new = !pre_exists;              // 之前不存在 → 新文件
                bool ft_changed = pre_exists && (ft != pre_ft);
                bool sz_changed = pre_exists && (sz != pre_size);

                if (is_new || ft_changed || sz_changed) {
                    // 再等到“大小稳定”以确保写完
                    if (sz == last_size) {
                        if (++stable_cnt >= 3) return true;     // 连续 3 次大小不变
                    }
                    else {
                        stable_cnt = 0;
                        last_size = sz;
    ...
    std::vector<std::string>& newLayer)
{
    ...
    //不会就是因为这个傻逼地方一直越界访问吧？？？？？？？？
    pinfo.resize(pads.size()); // 防止越界

    //加在这里吗？？？？？？？？？？？？？？？？？？？？？？？？？
    // === 强制让 newNet/newW/newLayer 的大小与 small.size() 对齐 ===
    if (newNet.size() != small.size() || newW.size() != small.size() || newLayer.size() != small.size()) {
        std::cerr << "[WARN] SeedSmallFromPads: vector size mismatch. "
            << "small=" << small.size()
            << " newNet=" << newNet.size()
            << " newW=" << newW.size()
            << " newLayer=" << newLayer.size() << std::endl;
        newNet.assign(small.size(), INT_MIN);
        newW.assign(small.size(), -1.0);
        newLayer.assign(small.size(), "");
    }
    //=======================
    ...
    for (size_t i = 0; i < pads.size(); ++i) {
        ...
        for (int s = 0; s < (int)small.size(); ++s) {
            ...
            // 仅当索引有效才读取/写入
            if ((size_t)s < newNet.size() && (size_t)s < newW.size() && (size_t)s < newLayer.size()) {
                ...
                std::cerr << "[BUG] SeedSmallFromPads: index s=" << s
                    << " out of range (small=" << small.size()
                    << ", newNet=" << newNet.size()
                    << ", newW=" << newW.size()
                    << ", newLayer=" << newLayer.size() << ")\n";
            }
        }
    }
}
...
inline RealRectMM ExpandRect_HalfPlusK(const RealRectMM& in, const Grid& grid, int kWholePixels) {
...
inline RealRectMM ClipRectToBoard(const RealRectMM& in,
...
inline RealRectMM CornersToRect_LT_RT_RB_LB(const std::vector<std::pair<double, double>>& c) {
...
// === 把矩形转回四角（LT,RT,RB,LB） ===
inline std::vector<std::pair<double, double>> RectToCorners_LT_RT_RB_LB(const RealRectMM& r) {
    return {
        { r.x_left , r.y_top },   // LT
...
int main() {
    try {
        // === 1) 初始化 Grid ===
        std::string filename = getenvOr("P5_INPUT_PCB", (kP5 + "/testcase.kicad_pcb"));

        //std::string filename = "testcase.kicad_pcb";   // 按需修改，这一行是写死的版本
        Grid grid;
		...
		// 从环境变量读取参数（方便脚本调用时传参）
        int layerId = std::stoi(getenvOr("P5_LAYER_ID", "1"));
        int blockedAreaId = std::stoi(getenvOr("P5_BLOCK_ID", "1"));
        int expandPixels = std::stoi(getenvOr("P5_EXPAND", "15"));
        int kWholePixels = std::stoi(getenvOr("P5_KWHOLE", "20"));
        ...
        //std::string outputDir = buildIterOutDir();// 从环境变量读取输出目录
        std::string outputDir = GetOutputDir();

        // 创建输出目录（如果不存在）
        // 注意：这里需要检查目录是否存在并创建，但为了避免引入额外依赖，我们先假设目录存在
        std::string weightsOutputFile = outputDir + "default.txt";
        ...
        // 同步一份给 rerouting/bin/default.txt
        CopyOverwrite(weightsOutputFile, kRR + "/bin/default.txt");
        std::cout << "[SYNC] 权重已拷贝到: " << (kRR + "/bin/default.txt") << "\n";
        ...
        */
        ...
        // ===== (新增) 同步一份到 rerouting/case（供 pcbrouter 直接使用）=====
        if (const char* case_path = std::getenv("P5_ROUTER_CASE")) {
            try {
                fs::create_directories(fs::path(case_path).parent_path());
                fs::copy_file(templateOutputFile, case_path, fs::copy_options::overwrite_existing);
                std::cout << "[Case] 写入: " << case_path << std::endl;
            }
            catch (const std::exception& e) {
                std::cerr << "[Case] 写入失败: " << e.what() << std::endl;
            }
        }
        std::cout.flush();
        ...
        //================================================================================================
        // === 把模板 & 权重送去 rerouting ===
        CopyOverwrite(templateOutputFile, kRR + "/case/complete_pcb_template.kicad_pcb");
        CopyOverwrite(outputDir + "default.txt", kRR + "/bin/default.txt");
        std::cout << "[SYNC] 模板 → " << (kRR + "/case/complete_pcb_template.kicad_pcb") << "\n";

        //多了“记录基线时间 + 等待新文件”
		//=================================================================================
        // 目标产物路径
        std::string rrOutAbs = kRR + "/bin/" + kRR_OUT_NAME;
        std::string rrOutArchived = outputDir + kRR_OUT_NAME;

        // 记录运行前的文件状态（用于判断“是否变化”）
        bool pre_exists = fs::exists(rrOutAbs);
        fs::file_time_type pre_ft{};
        uintmax_t pre_size = 0;
        if (pre_exists) {
            try {
                pre_ft = fs::last_write_time(rrOutAbs);
                pre_size = fs::file_size(rrOutAbs);
            }
            catch (...) {
                pre_exists = false; // 读取失败时按不存在处理
            }
        }

        // 构建命令：可多次构建
        std::string buildCmd =
            "bash -lc 'cd " + kRR + " && cmake -S . -B build && cmake --build build -j'";

        // 运行命令：把 stdout/stderr 同步写入本轮日志，便于排查
        std::string runCmd =
            "bash -lc 'cd " + kRR + " && "
            "(./build/bin/rerouting || ./bin/rerouting || ./bin/run) "
            "> " + outputDir + "rerouting.log 2>&1'";

        /*std::cout << "[Rerouting] build...\n";
        std::system(buildCmd.c_str());

        std::cout << "[Rerouting] run... (log: " << (outputDir + "rerouting.log") << ")\n";
        std::system(runCmd.c_str());*/

        // 等待“新或变化且大小稳定”的产物，避免误拿旧文件
        std::cout << "[Rerouting] 等待产物: " << rrOutAbs << std::endl;
        if (!WaitForNewOrChangedStableFile(rrOutAbs, pre_exists, pre_ft, pre_size)) {
            std::cerr << "[ERROR] rerouting 产物未生成或仍是旧文件。"
                "请查看日志: " << (outputDir + "rerouting.log") << "\\n";
            // 你可以在这里 return; 或者继续走降级路径（沿用 currentInput）
        }
        else {
            CopyOverwrite(rrOutAbs, rrOutArchived);
            std::cout << "[Rerouting产物] 已归档: " << rrOutArchived << "\\n";
        }
        ...
        // 收集并删除"第一步像素扩张形成的现实外沿矩形"内/边上的线段
        {
            ...
            if (!candIds.empty()) {
                ...
                if (!parser.parseFile(filename)) {
                    ...
                    std::string v5InputPath = outputDir + kRR_OUT_NAME; // 不写死
