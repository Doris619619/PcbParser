...
int main() {
    try {
        ...
        int expandPixels = 15;   // 像素域扩展像素数（有补偿）
        ...
        //9，测试删除的segment并生成新的kicad7.0文件
        // === [NEW] 收集并删除“第一步像素扩张形成的现实外沿矩形”内/边上的线段 ===
        {
            ...
            if (!candIds.empty()) {
                ...
                if (!parser.parseFile(filename)) {
                    ...
                    // 输出文件名：<原名去后缀>.firstrect_pruned.kicad_pcb
                    ...
                    // 注意：这里仍然基于最开始输入的 KiCad 7.0 testcase 所构建的 grid
                    ...
                    // ！！！这里是独立的 v5 输入文件（与最开始的 7.0 testcase 文件不同）！！！
                    std::string v5InputPath =
                        "D:\\梁彦诗的项目\\Liang的科研\\第五步最后9999999999999999999999999999999999999\\文件存储\\output.bestSolutionWithMerging.s_10_i_5_b_0_lc_500_to_15000_vo_40_po_2000_tss_0_vss_0.test1.kicad_pcb";
                    ...
                    // ====== (新增) 再次保存 —— 终版 v7 文件（存到 v5 文件所在目录）======
                    // 不引入 <filesystem>，用纯字符串处理目录和文件名
                    std::string outPath2Dir = v5InputPath;
                    {
                        size_t pos = outPath2Dir.find_last_of("/\\");
                        if (pos != std::string::npos) outPath2Dir = outPath2Dir.substr(0, pos);
                        else outPath2Dir = ".";
                    }
                    std::string outPath2Name = v5InputPath;
                    {
                        size_t pos = outPath2Name.find_last_of("/\\");
                        if (pos != std::string::npos) outPath2Name = outPath2Name.substr(pos + 1);
                        size_t dot2 = outPath2Name.rfind('.');
                        if (dot2 != std::string::npos) outPath2Name = outPath2Name.substr(0, dot2);
                        outPath2Name += ".merged_final_v7.kicad_pcb";
                    }
                    std::string outPath2 = outPath2Dir + "\\" + outPath2Name;
